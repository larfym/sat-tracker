...
void taskMotionControl(void *pvParameters)
{
  ...
  float output_az = 0.0f, output_el = 0.0f;

  int new_dir_az = 0, new_dir_el = 0;
  int current_dir_az = 0, current_dir_el = 0;
  ...
  for (;;)
  {
    ...
    // Constrain Elevation
    set_angle.elevation = (set_angle.elevation > MAX_ELEVATION_deg) ? MAX_ELEVATION_deg : (set_angle.elevation < 0.0f) ? 0.0f
                                                                                                                       : set_angle.elevation;
